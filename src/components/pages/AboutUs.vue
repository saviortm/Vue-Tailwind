<template>
  <div>
    <NavBar/>
    <div class="flex flex-col text-[#331B3B] mt-32 bg-[#ECF7FF] rounded-3xl">
      <div class="flex justify-center font-bold text-6xl tracking-[-0.01em] mt-10">ABOUT US IS A TEST PAGE</div>
      <div class="w-full h-96">
        <!-- Modal toggle -->
        <button @click="showModal" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button" data-modal-toggle="authentication-modal">
          Toggle modal
        </button>

        <!-- Main modal -->
        <div v-if="this.modal" id="authentication-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
