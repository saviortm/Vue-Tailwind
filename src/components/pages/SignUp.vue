                ...
                <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                    <div class="p-6 space-y-2 md:space-y-6 sm:p-8">
                        ...
                        <form @submit.prevent="handleSubmit" class="space-y-2 md:space-y-3" action="#">
                            <div>
                                <label for="fname" class="block mb-1 text-sm font-medium text-gray-900 ">Your first
                                    name</label>
                                <input v-model="state.first" type="text" name="name" id="fname"
                                       class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                       placeholder="first name" required="">
                                <span v-if="v$.first.$error">{{ v$.first.$errors[0].$message }}</span>
                            </div>
                            <div>
                                <label for="lname" class="block mb-1 text-sm font-medium text-gray-900 ">Your last
                                    name</label>
                                <input v-model="state.last" type="text" name="name" id="lname"
                                       class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                            ...
                            <div>
                                ...
                                <input v-model="state.password.password" type="password" name="password" id="password"
                                       placeholder="••••••••"
                                       class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                       required="">
                                <span
                                    v-if="v$.password.password.$error">{{ v$.password.password.$errors[0].$message }}</span>
                            </div>
                            <div>
                                <label for="confirm-password" class="block mb-1 text-sm font-medium text-gray-900 ">Confirm
                                    password</label>
                                <input v-model="state.password.confirm" type="password" name="confirm-password"
                                       id="confirm-password" placeholder="••••••••"
                                       class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                       required="">
                                <span
                                    v-if="v$.password.confirm.$error">{{ v$.password.confirm.$errors[0].$message }}</span>
                            </div>
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input @click="checkBox" id="terms" aria-describedby="terms" type="checkbox"
                            ...
                            <button @click="submitForm" type="submit"
                                    class="w-full text-white bg-[#331B3B] hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                                Create an account
                            </button>
                        </form>
                    </div>
